...
//------
    ...
    if(siguiente==horaFinSimulacion)proximoEvento(n,horas);
...
void proximoEvento(int n, int v[n]){    //v[0] es la hora actual, v1 es la prox llegada, v2 prox fin de servicio
...
if(siguiente > salida){ //salida antes que llegada,F.S. ,etc...

    v[0]=salida;
    procesarAbandono(v[0]);

}else if(siguiente == salida){ //empate

    v[0]=salida;
    procesarAbandono(v[0]);
    switch (indice){

    case 1: //el proximo evento es la llegada del cliente
        v[0] = v[1];
    if(v[1] == v[2]){ //hora de entrada de cliente y de fin de servicio coinciden
           ...
           desencolarPrioridad();
           cont++; //suma 1 a la lista de clientes atendidos
           ocupado=1;
    ...
    }else if(!ocupado && servidorEstado){//no hay nadie siendo atendido
            ocupado=1;
            v[2]=v[0]+horaServicio();
            v[1]=v[0] + horaLlegada();
    }else{
    ...
    case 2 :
        ...
        else if(clientesCola() > 0 || clientesColaPrioridad() > 0){
        v[2]=v[0]+horaServicio();
        desencolarPrioridad();
    }else{
        ocupado = 0;
    ...
    case 3:      //hora del descanso
        v[4]= v[3] + horaDescansando();
        v[0]=v[3];
        servidorEstado=0;
        if(v[3]==v[2])ocupado=0;
        if(ocupado) v[2]=v[2]+ (v[4]-v[3]);
    break;
    ...
    case 4:
        ...
        servidorEstado = 1;
        v[3]= v[0] + horaDescanso();
        if((clientesCola() > 0 || clientesColaPrioridad() > 0) && !ocupado){
        v[2]=v[0]+horaServicio();
        ocupado=1;
        desencolarPrioridad();
